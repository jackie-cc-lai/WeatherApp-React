...
export class http{
    ...
    private Init = async () => {
        try{
            if(this._token !== ''){
                ...
                if(response.token){
                    this._cookies.set('jwtToken', response.token);
    ...
    public Get = async (params:ServiceParams) => {
        ...
        if(params.method ==="Get" && !response){
            throw new Error(`No data retrieved from remote service`);
        }
        if(response){
    ...
    private Check = (params:ServiceParams, checkBody?:boolean) =>{
        if(!params.url){
            throw new Error(`Request does not have url associated`);
        }
        if(checkBody){
            if(!params.body){
                throw new Error(`Request does not have body associated`);
            }
        }
    }
}